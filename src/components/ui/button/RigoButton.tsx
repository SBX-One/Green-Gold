...
export default function RigoButton({ inv, text, icon = true, onClick, disabled }: RIGOBTNPROPS) {
    return (
        <button disabled={disabled} onClick={onClick} className={`${inv ? "bg-surface-primary border border-border-action" : "bg-white border border-border-default"} disabled:bg-surface-disabled w-full md:w-45 h-11 md:h-14 flex items-center justify-between px-6 rounded-2xl`}>
